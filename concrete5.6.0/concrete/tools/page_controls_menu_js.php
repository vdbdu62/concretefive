...
$(function() {
	...
	if (!$dh->inDashboard()) { ?>
		...
		<?php  if ($c->getCollectionPointerID() > 0) { ?>
			...
			item = new ccm_statusBarItem();
			item.setCSSClass('info');
			item.setDescription('<?php echo  t("This page is an alias of one that actually appears elsewhere.", $c->getCollectionCheckedOutUserName())?>');
		...
		if ($c->isMasterCollection()) { ?>
			...
			item = new ccm_statusBarItem();
			item.setCSSClass('info');
			item.setDescription('<?php echo  t('Page Defaults for %s Page Type. All edits take effect immediately.', $c->getCollectionTypeName()) ?>');
			ccm_statusBar.addItem(item);		
		<?php  } ?>
		<?php 
		...
		if ($cp->canViewToolbar()) { ?>
			<?php  if (is_array($workflowList)) { ?>
				<?php  foreach($workflowList as $wl) { ?>
					...
					item = new ccm_statusBarItem();
					item.setCSSClass('<?php echo $wr->getWorkflowRequestStyleClass()?>');
					item.setDescription('<?php echo $wf->getWorkflowProgressCurrentDescription($wl)?>');
					item.setAction('<?php echo $wl->getWorkflowProgressFormAction()?>');
					item.enableAjaxForm();
		...
		if (!$c->getCollectionPointerID() && !$hasPendingPageApproval) {
			if (is_object($vo)) {
				if (!$vo->isApproved() && !$c->isEditMode()) { ?>
					...
					item = new ccm_statusBarItem();
					item.setCSSClass('info');
					item.setDescription('<?php echo  t("This page is pending approval.")?>');
